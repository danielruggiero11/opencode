...
export async function openNewChat(client: CDPClient): Promise<void> {
  // Step 1: Click the split button chevron with real mouse events
  if (await client.clickSelector('[data-testid="newChatSplitButton"]')) {
    await sleep(1000) // Menu needs ~1s to render

    // Step 2: Click "Temporary chat" with real mouse events
    if (await client.clickSelector('[data-testid="newPrivateChatButton"]')) {
      await sleep(1200)
      // Re-establish execution context after page navigation
      await client.send("Runtime.enable", {})
      await sleep(300)
      return
    }

    // Menu opened but temp chat not found — dismiss
    await client.pressKey("Escape", "Escape", 27)
    await sleep(300)
  }

  // Fallback: regular new chat (persists to history)
...
export async function setEffort(client: CDPClient, effort: string): Promise<void> {
  ...
  if (switcherVisible) {
    ...
    if (!await client.clickSelector("#gptModeSwitcher")) return
    await sleep(700)
  ...
  // Radio not found — try expanding a model submenu
  const expanded = await client.evaluate(`
    (() => {
      const items = [...document.querySelectorAll('[role="menuitem"]')];
      const trigger = items.find(e => {
        const t = (e.innerText||'').split('\\n')[0].trim().toLowerCase();
        return t === 'auto' || t === 'quick response' || t === 'think deeper' || t === 'opus' || t.includes('model');
      });
    ...
    if (wasGenerating) {
      if (!isGenerating && text) {
        ...
        if (stopGoneCount >= SETTLE_AFTER_STOP) {
          console.error(`[cdp-web] awaitResponse: stop gone + settled (${stopGoneCount} polls). Returning ${textLen} chars.`)
          return text
        }
        // Early confirm: Send button back AND turnDone
        if (hasSendBtn && turnDone) {
          console.error(`[cdp-web] awaitResponse: stop gone + send + turnDone. Returning ${textLen} chars.`)
          return text
        }
      }
      // While generating or settling, just keep polling
      last = text
      lastLen = textLen
      continue
    }
...
async function clickRadioItem(client: CDPClient, label: string): Promise<string> {
  const info = await client.evaluate(`
    (() => {
      ...
      if (!it) return JSON.stringify({status:'missing'});
